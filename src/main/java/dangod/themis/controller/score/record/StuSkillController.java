...
@CrossOrigin
@RestController
@RequestMapping(value = "/stu/skill")
public class StuSkillController extends BaseController{
    @Autowired
    ...
    @ApiOperation(value = "用户获取学生Skill信息(按学期)")
    @ContainAuthority(SELF_STUDENT)
    @Authorization
    public String getStudentSkillByTerm(HttpServletRequest request, HttpServletResponse response,
        ...
        List<SkillVo> list = recordService.getSkillByUserIdAndTerm(getUserId(request), term, page, size);
        if(list == null)
    ...
    @ApiOperation(value = "校级管理员获取学生Skill信息(按学期)")
    @ContainAuthority(SCHOOL_STUDENT_MANAGE)
    @Authorization
    public String schoolAdminGetStudentSkillByTerm(HttpServletRequest request, HttpServletResponse response,
                                    @RequestHeader(AUTHORIZATION)String token,
        ...
        if(getParameter(request, "size") != null)
            size = Integer.parseInt(getParameter(request, "size"));
        List<SkillVo> list = recordService.getSkillByStuIdAndTerm(stuId, term, page, size);
        if(list == null)
            return Result.send(FAIL, null, STU_FAIL_MESSAGE);
        return Result.send(SUCCESS, list, STU_SUCCESS_MESSAGE);
    }
    ...
    @ApiOperation(value = "班级管理员获取学生Skill信息(按学期)")
    @ContainAuthority(CLASS_STUDENT_MANAGE)
    @Class
    @Authorization
    public String classAdminGetStudentSkillByTerm(HttpServletRequest request, HttpServletResponse response,
                                    @RequestHeader(AUTHORIZATION)String token,
                                    @RequestParam("stuId")String stuId,
                                    @RequestParam("term")String term,
                                    @RequestParam("page")Integer page){
    ...
    @ApiOperation(value = "专业管理员获取学生Skill信息(按学期)")
    @ContainAuthority(MAJOR_STUDENT_MANAGE)
    @Major
    @Authorization
    public String majorAdminGetStudentSkillByTerm(HttpServletRequest request, HttpServletResponse response,
                                    @RequestHeader(AUTHORIZATION)String token,
                                    @RequestParam("stuId")String stuId,
                                    @RequestParam("term")String term,
                                    @RequestParam("page")Integer page){
