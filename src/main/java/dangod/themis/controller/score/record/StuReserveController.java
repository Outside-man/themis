...
@CrossOrigin
@RestController
@RequestMapping(value = "/stu/reserve")
public class StuReserveController extends BaseController{
    @Autowired
    ...
    @ApiOperation(value = "用户获取学生Reserve信息(按学期)")
    @ContainAuthority(SELF_STUDENT)
    @Authorization
    public String getStudentReserveByTerm(HttpServletRequest request, HttpServletResponse response,
        ...
        List<ReserveVo> list = recordService.getReserveByUserIdAndTerm(getUserId(request), term, page, size);
        if(list == null)
    ...
    @ApiOperation(value = "校级管理员获取学生Reserve信息(按学期)")
    @ContainAuthority(SCHOOL_STUDENT_MANAGE)
    @Authorization
    public String schoolAdminGetStudentReserveByTerm(HttpServletRequest request, HttpServletResponse response,
                                    @RequestHeader(AUTHORIZATION)String token,
        ...
        if(getParameter(request, "size") != null)
            size = Integer.parseInt(getParameter(request, "size"));
        List<ReserveVo> list = recordService.getReserveByStuIdAndTerm(stuId, term, page, size);
        if(list == null)
            return Result.send(FAIL, null, STU_FAIL_MESSAGE);
        return Result.send(SUCCESS, list, STU_SUCCESS_MESSAGE);
    }
