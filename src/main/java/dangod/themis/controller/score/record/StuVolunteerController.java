...
@CrossOrigin
@RestController
@RequestMapping(value = "/stu/volunteer")
public class StuVolunteerController extends BaseController{
    @Autowired
    ...
    @ApiOperation(value = "用户获取学生Volunteer信息(按学期)")
    @ContainAuthority(SELF_STUDENT)
    @Authorization
    public String getStudentVolunteerByTerm(HttpServletRequest request, HttpServletResponse response,
        ...
        List<VolunteerVo> list = recordService.getVolunteerByUserIdAndTerm(getUserId(request), term, page, size);
        if(list == null)
    ...
    @ApiOperation(value = "校级管理员获取学生Volunteer信息(按学期)")
    @ContainAuthority(SCHOOL_STUDENT_MANAGE)
    @Authorization
    public String schoolAdminGetStudentVolunteerByTerm(HttpServletRequest request, HttpServletResponse response,
                                    @RequestHeader(AUTHORIZATION)String token,
        ...
        if(getParameter(request, "size") != null)
            size = Integer.parseInt(getParameter(request, "size"));
        List<VolunteerVo> list = recordService.getVolunteerByStuIdAndTerm(stuId, term, page, size);
        if(list == null)
            return Result.send(FAIL, null, STU_FAIL_MESSAGE);
        return Result.send(SUCCESS, list, STU_SUCCESS_MESSAGE);
    }
    ...
    @ApiOperation(value = "班级管理员获取学生Volunteer信息(按学期)")
    @ContainAuthority(CLASS_STUDENT_MANAGE)
    @Class
    @Authorization
    public String classAdminGetStudentVolunteerByTerm(HttpServletRequest request, HttpServletResponse response,
                                    @RequestHeader(AUTHORIZATION)String token,
                                    @RequestParam("stuId")String stuId,
                                    @RequestParam("term")String term,
                                    @RequestParam("page")Integer page){
    ...
    @ApiOperation(value = "专业管理员获取学生Volunteer信息(按学期)")
    @ContainAuthority(MAJOR_STUDENT_MANAGE)
    @Major
    @Authorization
    public String majorAdminGetStudentVolunteerByTerm(HttpServletRequest request, HttpServletResponse response,
                                    @RequestHeader(AUTHORIZATION)String token,
                                    @RequestParam("stuId")String stuId,
                                    @RequestParam("term")String term,
                                    @RequestParam("page")Integer page){
