...
@CrossOrigin
@RestController
@RequestMapping(value = "/stu/${entity?uncap_first}")
public class Stu${entity}Controller extends BaseController{
    @Autowired
    ...
    @ApiOperation(value = "用户获取学生${entity}信息(按学期)")
    @ContainAuthority(SELF_STUDENT)
    @Authorization
    public String getStudent${entity}ByTerm(HttpServletRequest request, HttpServletResponse response,
        ...
        List<${entity}Vo> list = recordService.get${entity}ByUserIdAndTerm(getUserId(request), term, page, size);
        if(list == null)
    ...
    @ApiOperation(value = "校级管理员获取学生${entity}信息(按学期)")
    @ContainAuthority(SCHOOL_STUDENT_MANAGE)
    @Authorization
    public String schoolAdminGetStudent${entity}ByTerm(HttpServletRequest request, HttpServletResponse response,
                                    @RequestHeader(AUTHORIZATION)String token,
        ...
        if(getParameter(request, "size") != null)
            size = Integer.parseInt(getParameter(request, "size"));
        List<${entity}Vo> list = recordService.get${entity}ByStuIdAndTerm(stuId, term, page, size);
        if(list == null)
            return Result.send(FAIL, null, STU_FAIL_MESSAGE);
        return Result.send(SUCCESS, list, STU_SUCCESS_MESSAGE);
    }
